...
void main() {
  ...
  if (e == 1.0) {
    float line = sin((uv.y * resolution.y * 0.18) + sin(uv.x * 18.0 + time) * 4.0);
    color *= smoothstep(-0.35, 0.65, line);
  } else if (e == 2.0) {
    vec2 p = floor(gl_FragCoord.xy);
    float b = mod(p.x + p.y * 2.0, 4.0) / 4.0;
    float q = step(b, l);
    color = mix(vec3(q), color, 0.28);
  } else if (e == 3.0 || e == 5.0) {
    float cell = e == 3.0 ? 9.0 : 12.0;
    vec2 gv = fract(gl_FragCoord.xy / cell) - 0.5;
    float dotMask = smoothstep(0.36 * (1.15 - l), 0.34 * (1.15 - l), length(gv));
    color = mix(vec3(0.0), color, dotMask);
  } else if (e == 4.0) {
    float shift = (hash(vec2(floor(uv.y * resolution.y / 4.0), floor(time * 8.0))) - 0.5) * 0.035 * amt;
    color = texture(inputTexture, vec2(clamp(uv.x + shift * (1.0 - l), 0.0, 1.0), uv.y)).rgb;
  } else if (e == 6.0) {
    float bands = max(2.0, 8.0 * amt);
    float q = floor(l * bands) / bands;
    float edge = smoothstep(0.015, 0.0, abs(fract(l * bands) - 0.5));
    color = mix(vec3(q), vec3(0.0), edge * 0.7);
  } else if (e == 7.0) {
    float l1 = lum(texture(inputTexture, uv + vec2(px.x, 0.0)).rgb);
    float l2 = lum(texture(inputTexture, uv - vec2(px.x, 0.0)).rgb);
    float l3 = lum(texture(inputTexture, uv + vec2(0.0, px.y)).rgb);
    float l4 = lum(texture(inputTexture, uv - vec2(0.0, px.y)).rgb);
    float edge = clamp(abs(l1 - l2) + abs(l3 - l4), 0.0, 1.0);
    color = vec3(smoothstep(0.08, 0.28, edge));
  } else if (e == 8.0) {
    float darkness = 1.0 - l;
    float h1 = step(0.48, fract((uv.x + uv.y) * 42.0));
    float h2 = step(0.52, fract((uv.x - uv.y) * 42.0));
    float hatch = mix(1.0, min(h1, h2), smoothstep(0.25, 0.9, darkness));
    color *= hatch;
  } else if (e == 9.0) {
    float blocks = max(12.0, 80.0 / amt);
    vec2 buv = floor(uv * blocks) / blocks + 0.5 / blocks;
    color = texture(inputTexture, buv).rgb;
  } else if (e == 10.0) {
    color = vec3(step(0.5, l));
  } else if (e == 11.0) {
    float n = hash(floor(uv * resolution / 6.0) + floor(time * 12.0));
    vec2 duv = uv + (vec2(n, hash(uv * 91.0)) - 0.5) * 0.025 * amt;
    color = mix(texture(inputTexture, clamp(duv, vec2(0.0), vec2(1.0))).rgb, vec3(n), 0.22);
  } else if (e == 12.0) {
    float rain = step(0.86, hash(vec2(floor(uv.x * 80.0), floor(uv.y * 70.0 - time * 25.0))));
    color = mix(color * vec3(0.25, 0.7, 0.25), vec3(0.2, 1.0, 0.25), rain * 0.85);
  } else if (e == 13.0) {
    float scan = 0.85 + 0.15 * sin(uv.y * resolution.y * 3.14159);
    float jitter = (hash(vec2(floor(uv.y * 120.0), floor(time * 20.0))) - 0.5) * 0.012 * amt;
    color.r = texture(inputTexture, vec2(clamp(uv.x + jitter + 0.003, 0.0, 1.0), uv.y)).r;
    color.g = texture(inputTexture, vec2(clamp(uv.x + jitter, 0.0, 1.0), uv.y)).g;
    color.b = texture(inputTexture, vec2(clamp(uv.x + jitter - 0.003, 0.0, 1.0), uv.y)).b;
    color *= scan;
  } else if (e == 14.0) {
    vec2 cell = floor(uv * 24.0);
    vec2 f = fract(uv * 24.0);
    float d = 1.0;
    for (int y = -1; y <= 1; y++) {
      for (int x = -1; x <= 1; x++) {
        vec2 n = vec2(float(x), float(y));
        vec2 p = vec2(hash(cell + n), hash(cell + n + 19.7));
        d = min(d, length(n + p - f));
      }
    }
    color = mix(color, vec3(smoothstep(0.08, 0.12, d)), 0.65);
  }
